...
const HabitTracker = () => {
  ...
  const [selectedHabitForCalendar, setSelectedHabitForCalendar] =
  ...
  // Removed unused getWeekdayName function
  ...
  const getCompletionRate = (habit: Habit, days: string[]): string => {
  ...
  const getCurrentStreak = (habit: Habit) => {
    ...
    for (let i = 0; i < 365; i++) {
      ...
      if (habit.completions[dateStr]) streak++;
      else break;
    }
    return streak;
  };
  ...
  const getChartData = (days: string[]) => {
    return days.map((date) => {
      ...
      habits.forEach((habit) => {
        entry[habit.name] = habit.completions[date] ? 1 : 0;
      });
      return entry;
    });
  };

  const getWeeklyData = (days: string[]) => {
    return habits.map((habit) => {
      const completed = days.filter((day) => habit.completions[day]).length;
      return {
        name: habit.name,
        completed: days.length > 0 ? (completed / days.length) * 100 : 0,
        color: habit.color,
      };
    });
  };

  const getCompletionDistribution = (days: string[]) => {
    ...
    // Filter out items with 0 value to clean up the pie chart
  ...
  const statsDateRange =
    dateRange.start && dateRange.end ? getDateRangeDays() : weekDays;

  // Now these functions exist, so these variables will populate correctly
  const chartData = getChartData(statsDateRange);
  const weeklyData = getWeeklyData(statsDateRange);
  const pieData = getCompletionDistribution(statsDateRange);
              ...
              <div className="flex justify-between items-center mb-4">
                ...
                <div className="text-center">
                  <div className="text-base md:text-lg font-bold text-gray-800">
                    ...
                    -{" "}
                    {weekDays[6] &&
                      new Date(weekDays[6]).toLocaleDateString("en-US", {
                        month: "short",
                        day: "numeric",
                      })}
                  </div>
            ...
            {habits.length === 0 ? (
              ...
              <div className="space-y-3 md:space-y-4">
                {habits.map((habit) => (
                  <div
                    key={habit.id}
                    className="bg-white rounded-2xl shadow-2xl p-4 md:p-6 transform hover:scale-105 transition-all hover:shadow-purple-200"
                  >
            ...
            <div className="bg-white rounded-2xl shadow-2xl p-4 md:p-6">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 md:gap-4">
                {habits.map((habit) => (
                  <div
                    key={habit.id}
                    className="border-2 border-purple-100 rounded-xl p-3 md:p-4 hover:border-purple-300 transition hover:shadow-lg"
                  >
                ...
                <div className="grid grid-cols-7 gap-1 md:gap-2">
                  {calendarDays.map((date, index) => (
                    <button
                      ...
                      onClick={() =>
                        date &&
                        toggleCompletion(selectedHabitForCalendar.id, date)
                      }
                      disabled={!date}
                  ...
                  <div className="grid grid-cols-2 gap-4 text-sm">
                    <div>
                      ...
                      <div className="text-xl font-bold text-purple-600">
                        {
                          calendarDays.filter(
                            (d) => d && selectedHabitForCalendar.completions[d]
                          ).length
                        }
                      </div>
                    </div>
                    <div>
                      ...
                      <div className="text-xl font-bold text-blue-600">
                        {getCompletionRate(
                          ...
                          // FIXED: Added type predicate to ensure array contains only strings
                          calendarDays.filter((d): d is string => d !== null)
                        )}
                        %
                      </div>
                    </div>
